...
func TestNewDockerfileReferenceReturnsSourcePathAndIdentifier(t *testing.T) {
	got, err := newDockerfileReference(`../build\Dockerfile.custom`)
	require.NoError(t, err)

	assert.Equal(t, `../build\Dockerfile.custom`, got.sourcePath)
	assert.Equal(t, "resource/dockerfile/Dockerfile.custom", got.identifier)
}
