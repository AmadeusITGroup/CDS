...
var (
	commentLineRegex = regexp.MustCompile(`(?m:^\s*//.*$)`)
)

// Config holds one parsed devcontainer configuration.
//
// Do not reintroduce a defaultConfig-style singleton here. The previous CLI
// architecture parsed one configuration for one process invocation, but the
// agent can serve multiple requests concurrently; keeping configuration
// request-scoped prevents one request from mutating another request's state.
type Config struct {
	v *viper.Viper
}
...
func (c *Config) LoadFromBytes(dataReader io.Reader) error {
	...
	uncommented := commentLineRegex.ReplaceAll(data, nil)
